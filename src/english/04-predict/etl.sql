...
-- ETL to generate the most recent prediction base
...
SELECT 
    ...
    -- Customer growth/derived metrics
    ...
    -- Store characteristics (constant or rarely change)
    t3.CompetitionOpen,
    t3.Promo2Open,
    t3.StoreType,
    t3.Assortment,
    ...
    -- Calendar and temporal indicators
...
LEFT JOIN fs_customer  AS t2 ON t1.IdStore = t2.IdStore AND t1.DtRef = t2.DtRef
LEFT JOIN fs_store     AS t3 ON t1.IdStore = t3.IdStore AND t1.DtRef = t3.DtRef
LEFT JOIN fs_temporal  AS t4 ON t1.IdStore = t4.IdStore AND t1.DtRef = t4.DtRef

-- Only most recent data for all IdStore
WHERE t1.DtRef = (SELECT MAX(DtRef) FROM fs_sales)